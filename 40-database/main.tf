  #so this script meaning is creating instance trough terraform and doing configration trrough remote,from ansible  #
...
# null resource for mongodb
resource "terraform_data" "mongodb" { #this is null resource it will not create any instance just used for connecting the instance
    triggers_replace = [
...
resource "aws_instance" "mysql" { # Create an redis instance for the database
  ...
  subnet_id = local.database_subnet_id[0] # Use the first private subnet ID from local variables
  iam_instance_profile = "EC2RoleToFetchSSMParms"
  tags = merge(
    local.common_tags,
    {
...
resource "terraform_data" "mysql" { #thia is null resource it will not creat any instance just used for connecting the instance 
  ...
  provisioner "file" {  
   ...
   # this block allows you to automatically configure your ec2 instance after its created in this case, to install and configure a MYSQL
     
     provisioner "remote-exec" {
       inline = [ 
       "chmod +x /tmp/bootstrap.sh",
       "sudo sh /tmp/bootstrap.sh mysql dev ${var.environment}"
        ]
     }
...
resource "terraform_data" "rabbitmq" { #thia is null resource it will not creat any instance just used for connecting the instance 
  ...
  provisioner "file" {  #file is for copy
   ...
   connection {
     ...
     #This block allows you to automatically configure your EC2 instance after it’s created — in this case, to install and configure a MySQL service using Ansible.
     provisioner "remote-exec" {
       inline = [ 
       "chmod +x /tmp/bootstrap.sh",
...
resource "aws_route53_record" "mongodb" {
  zone_id = var.zone_id
  name    = "mongodb-${var.environment}.${var.zone_name}" #mongodb-dev.spandanas.click
  type    = "A"
  ttl     = 1
